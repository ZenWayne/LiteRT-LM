...
std::vector<std::string> SentencePieceTokenizer::GetTokens() const {
  // LLGuidance (the consumer of GetTokens via LlgConstraintProvider) requires
  // control/special tokens to be prefixed with the SPECIAL_TOKEN_MARKER byte
  // 0xFF so its token-trie can resolve them as special tokens. The
  // constrained-decoding Lark grammar references FC control tokens (e.g.
  // "<|tool_call>", "<|\"|>") via <special_token> rules; without the marker
  // llguidance has no special-token-prefix node and panics ("missing special
  // token prefix"). 0xFF is not a valid UTF-8 byte, so it never collides with
  // a normal piece. CONTROL (e.g. <bos>/<eos>) and USER_DEFINED (the chat/FC
  // control tokens) are exactly the special ones.
  ...
  for (const auto& piece : processor_->model_proto().pieces()) {
    if (piece.type() == sentencepiece::ModelProto::SentencePiece::CONTROL ||
        piece.type() == sentencepiece::ModelProto::SentencePiece::USER_DEFINED) {
      tokens.push_back(std::string(1, kSpecialTokenMarker) +
                       std::string(piece.piece()));
    } else {
      tokens.push_back(std::string(piece.piece()));
    }
  }
  return tokens;
}
